...
class BaseVAE(nn.Module):
    def __init__(self, img_size, z_dim):
        ...
        down_factory = 4
        self.conv_output_size = [16 * (2 ** (down_factory - 1))] + [size // (2 ** down_factory) for size in img_size[-2:]]
        print(self.conv_output_size)

        self.enc_block = nn.Sequential(OrderedDict([
                            ("conv_layer_00", ConvLayer(img_size[0], 16, 3, 2, 1)),
                            ("conv_layer_01", ConvLayer(16, 16, 3, 1, 1)),
                            ("conv_layer_10", ConvLayer(16, 32, 3, 2, 1)),
                            ("conv_layer_11", ConvLayer(32, 32, 3, 1, 1)),
                            ("conv_layer_20", ConvLayer(32, 64, 3, 2, 1)),
                            ("conv_layer_21", ConvLayer(64, 64, 3, 1, 1)),
                            ("conv_layer_30", ConvLayer(64, 128, 3, 2, 1)),
                            ("conv_layer_31", ConvLayer(128, 128, 3, 1, 1)),
        ...
        self.dec_block = nn.Sequential(OrderedDict([
                            ...
                            ("upsample_layer_01", ConvLayer(128, 128, 3, 1, 1)),
                            ("upsample_layer_02", ConvLayer(128, 64, 3, 1, 1)),
                            ("upsample_layer_10", nn.Upsample(scale_factor=2)),
                            ("upsample_layer_11", ConvLayer(64, 64, 3, 1, 1)),
                            ("upsample_layer_12", ConvLayer(64, 32, 3, 1, 1)),
                            ("upsample_layer_20", nn.Upsample(scale_factor=2)),
                            ("upsample_layer_21", ConvLayer(32, 32, 3, 1, 1)),
                            ("upsample_layer_22", ConvLayer(32, 16, 3, 1, 1)),
                            ("upsample_layer_30", nn.Upsample(scale_factor=2)),
                            ("upsample_layer_31", ConvLayer(16, 16, 3, 1, 1, act=None)),
                            ("upsample_layer_32", ConvLayer(16, img_size[0], 3, 1, 1, act=None)),
                        ]))
